...
struct LocationEmitter::Impl {
  /// The stream to which location information is emitted.
  llvm::raw_string_ostream &os;
  /// The style in which location information is emitted.
  ...
  void emitLocationInfo(Location loc) {
    llvm::TypeSwitch<Location, void>(loc)
        ...
        .Default([&](auto) { /* Don't print anything for unhandled locations. */ });
  }
...
LocationEmitter::LocationEmitter(LoweringOptions::LocationInfoStyle style, Location loc) {
  SmallPtrSet<Attribute, 8> locationSet;
  locationSet.insert(dyn_cast<LocationAttr>(loc));
...
void LocationEmitter::emitLocationSetInfo(
    llvm::raw_string_ostream &os, LoweringOptions::LocationInfoStyle style,
    const SmallPtrSetImpl<Attribute> &locationSet) {
  if (style == LoweringOptions::LocationInfoStyle::None)
    return;
  std::string resstr;
  llvm::raw_string_ostream sstr(resstr);
  Impl(sstr, style, locationSet);
